...
export class HotUpdate extends Component {
    ...
    onLoad() {
        if (sys.isNative) {
            ...
            this.lv.data.prompt = ikun.i18n.getLangByID("update_tips_check_update");
            this.startHotUpdate();
        }
    }
    ...
    private startHotUpdate() {
        ...
        options.onUpdateProgress = (event: jsb.EventAssetsManager) => {
            ...
            if (total == '0K') {
                ...
                this.lv.data.prompt = ikun.i18n.getLangByID("update_tips_update") + have + '/' + total + ' (' + parseInt(pc * 100 + "") + '%)';
            }
        ...
        options.onNeedToUpdate = (data: any, totalBytes: number) => {
            this.lv.data.prompt = ikun.i18n.getLangByID("update_tips_new_version");
            let total: string = "";
        ...
        options.onUpdateFailed = () => {
            this.lv.data.prompt = ikun.i18n.getLangByID("update_tips_update_fail");
            this.hot.checkUpdate();
        };
        options.onUpdateSucceed = () => {
            this.lv.data.progress = 100;
            this.lv.data.prompt = ikun.i18n.getLangByID("update_tips_update_success");
    ...
    private checkForceUpdate(callback: Function) {
        ...
        ikun.gui.open(UIID.Confirm, operate);
    }
    ...
    private showUpdateDialog(size: string, callback: Function) {
        ...
        tips.alert(ikun.i18n.getLangByID("update_nowifi_tip") + size, callback);
    }
}