...
/** 倒计时标签 */
@ccclass("LabelTime")
@menu('IkunFramework/Label/LabelTime （倒计时标签）')
export default class LabelTime extends Label {
    ...
    setTimeStamp(timeStamp: number) {
        this.countDown = TimeUtil.secsBetween(ikun.timer.getServerTime(), timeStamp);
        this.timing_end();
        this.timing_start();
        this.format();
    }
    ...
    private onGameShow() {
        const interval = Math.floor((ikun.timer.getTime() - (this.backStartTime || ikun.timer.getTime())) / 1000);
    ...
    private onGameHide() {
        this.backStartTime = ikun.timer.getTime();
    }
