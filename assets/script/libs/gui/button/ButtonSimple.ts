...
@menu('IkunFramework/Button/ButtonSimple （节点按钮）')
export default class ButtonSimple extends Component {
    ...
    protected async playEffect() {
        if (this.effect) {
            ikun.audio.playEffect(this.effect);
            // const effectId = await ikun.audio.playEffect(this.effect, resLoader.defaultBundleName, () => {
    ...
    onDestroy() {
        ...
        //         ikun.audio.putEffect(effectId, this.effect);
        //     });
        // }
    }
}
