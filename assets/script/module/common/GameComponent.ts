...
export class GameComponent extends Component {
   ...
   * @param path 资源路径
   */
  createPrefabNode(path: string, bundleName: string = ikun.res.defaultBundleName): Node {
   ...
   */
  createPrefabNodeAsync(path: string, bundleName: string = ikun.res.defaultBundleName): Promise<Node> {
  ...
  getRes<T extends Asset>(path: string, type?: __private.__types_globals__Constructor<T> | null, bundleName?: string): T | null {
    return ikun.res.get(path, type, bundleName);
  }
  ...
  private addPathToRecord<T>(
    ...
    if (paths instanceof Array) {
      ...
      let realBundle = ikun.res.defaultBundleName;
  ...
  load<T extends Asset>(
    ...
    ikun.res.load(bundleName, paths, type, onProgress, onComplete);
  }
  ...
  loadAsync<T extends Asset>(
    ...
    this.addPathToRecord(ResType.Load, bundleName, paths);
    return ikun.res.loadAsync(bundleName, paths, type);
  }
  ...
  loadDir<T extends Asset>(
    ...
    if (typeof dir === "string") {
      ...
      realDir = bundleName;
      realBundle = ikun.res.defaultBundleName;
    }

    this.addPathToRecord(ResType.LoadDir, realBundle, realDir);
    ikun.res.loadDir(bundleName, dir, type, onProgress, onComplete);
  }
    ...
    if (this.resPaths) {
      ...
      if (rps) {
        rps.forEach((value: ResRecord) => {
          ikun.res.release(value.path, value.bundle);
        });
        rps.clear();
      }
    }
  ...
  releaseDir() {
    if (this.resPaths) {
      ...
      if (rps) {
        rps.forEach((value: ResRecord) => {
          ikun.res.releaseDir(value.path, value.bundle);
        });
      }
    }
  }
  ...
  releaseAudioEffect() {
    if (this.resPaths) {
      ...
      if (rps) {
        rps.forEach((value: ResRecord) => {
          ikun.audio.putEffect(value.resId!, value.path, value.bundle);
        });
      }
    }
  }
  ...
  playMusic(url: string, callback?: Function, bundleName?: string) {
    ikun.audio.playMusic(url, callback, bundleName);
  }
  ...
  playMusicLoop(url: string, bundleName?: string) {
    ikun.audio.stopMusic();
    ikun.audio.playMusicLoop(url, bundleName);
  }
  ...
  protected setEvent(...args: string[]) {
    ...
    for (const name of args) {
      ...
      if (func) this.on(name, func, this);
      else console.error(`名为【${name}】的全局事方法不存在`);
    }
  }
