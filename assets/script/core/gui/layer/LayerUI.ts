...
export class LayerUI extends Node {
  ...
  protected async load(vp: ViewParams, bundle?: string) {
    // 加载界面资源超时提示
    const timerId = setTimeout(this.onLoadingTimeoutGui, ikun.config.game.loadingTimeoutGui);
  ...
  protected async showUi(vp: ViewParams): Promise<boolean> {
    ...
    if (r) {
      ...
      console.warn(
        `路径为【${vp.config.prefab}】的自定义预处理逻辑异常.检查预制上绑定的组件中 onAdded 方法,返回true才能正确完成窗口显示流程`
      );
      this.failure(vp);
    }
    return r;
