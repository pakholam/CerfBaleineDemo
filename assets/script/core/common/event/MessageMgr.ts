...
export class RoleViewComp extends Component{
    onLoad(){
        // 监听全局事件
        ikun.message.on(GameEvent.GameServerConnected, this.onHandler, this);
    }
    
    protected onDestroy() {
        // 对象释放时取消注册的全局事件
        ikun.message.off(GameEvent.GameServerConnected, this.onHandler, this);
    }
...
export class RoleViewComp extends Component{
    onLoad(){
        ...
        ikun.message.once(GameEvent.GameServerConnected, this.onHandler, this);
    }
...
export const message = new MessageMgr();
