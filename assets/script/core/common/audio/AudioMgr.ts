 ...
 * 音频管理
 * @example
 // 模块功能通过 ikun.audio 调用
 ikun.audio.playMusic("audios/nocturne");
 */
export class AudioMgr extends Component {
    ...
    save() {
        ...
        ikun.storage.set(LOCAL_STORE_KEY, this.local_data);
    }
    ...
    load() {
        ...
        this.local_data = ikun.storage.getJson(LOCAL_STORE_KEY);
        if (this.local_data) {
            try {
                this.setState();
            }
